...
def build_rag_evidence_pack(country: str, sector: str, keywords: str, docs: pd.DataFrame, design_assumptions: dict | None = None) -> str:
    if docs.empty:
    ...
    assumption_inventory = "\n".join(design_assumption_lines(design_assumptions))
...
아래 항목은 공공데이터의 관측값이 아니라 사용자가 조정할 수 있는 잠정 설계값입니다.
...
def design_assumption_lines(values: dict | None = None) -> list[str]:
    ...
    return [
        f"- [A01] 사업기간: 잠정 {int(assumptions['duration_months'])}개월",
        f"- [A02] 교육대상: 잠정 {int(assumptions['training_min'])}~{int(assumptions['training_max'])}명",
        f"- [A03] KPI: 잠정 {int(assumptions['kpi_min'])}~{int(assumptions['kpi_max'])}개",
        f"- [A04] 파트너 수: {safe_text(assumptions['partner_count'])}",
        f"- [A05] 예산 범위: {safe_text(assumptions['budget_range'])}",
        f"- [A06] 사업 단계: {safe_text(assumptions['project_stage'])}",
        f"- [A07] 주요 성과목표: {safe_text(assumptions['outcome_goal'])}",
    ]
...
- {sector} 분야 판단은 선택 분야와 직접 일치하는 CPS·KOICA 근거만 사용하며, 국가 공통 WDI와 파생점수는 보조 신호로 구분합니다 {cps_cites or project_cites}.
...
WDI 지표는 국가의 전반적인 개발·인프라·사회여건을 설명하는 보조 자료이며, {sector} 사업의 직접 수요를 단독으로 입증하지 않습니다.
...
## 9. AI Design Assumption · AI 생성 예비 설계 가정
아래 기간·인원·성과목표는 현지조사 이전의 잠정 예시이며, 수요조사·예산·수행기관 협의를 거쳐 수정해야 합니다.

{chr(10).join(design_assumption_lines(assumptions))}
...
def build_policy_brief(country: str, sector: str, row: pd.Series, docs: pd.DataFrame, design_assumptions: dict | None = None) -> str:
    assumptions = normalize_design_assumptions(design_assumptions)
...
## AI 생성 예비 설계 가정
- 잠정 사업기간: {int(assumptions['duration_months'])}개월
- 잠정 교육대상: {int(assumptions['training_min'])}~{int(assumptions['training_max'])}명
- 잠정 KPI: {int(assumptions['kpi_min'])}~{int(assumptions['kpi_max'])}개
- 파트너 수·예산: 현지조사와 수행기관 협의 후 확정
...
def builder_output_quality_report(
    ...
    assumption_separated = "AI 생성 예비 설계 가정" in proposal and all(f"[A{index:02d}]" in proposal for index in range(1, 8))
    evidence_classes_visible = all(label in proposal for label in ("Source Evidence", "Supplementary Source", "Derived Evidence", "Model Output", "AI Design Assumption"))
    wdi_role_visible = "국가 개발여건 보조 신호" in proposal and "직접 수요를 단독으로 입증하지 않습니다" in proposal
    missing_classes = int(docs.get("Evidence_Class", pd.Series(index=docs.index, dtype=str)).fillna("").eq("").sum())
    ...
    checks = [
        ...
        ["AI 설계 가정", "별도 분리", "A01~A07 표시" if assumption_separated else "필수 섹션·ID 누락", "REVIEW" if assumption_separated else "BLOCK"],
        ["원천·파생근거 구분", "5개 Evidence Class 표시", "표시" if evidence_classes_visible else "표시 누락", "PASS" if evidence_classes_visible else "BLOCK"],
        ["WDI 역할", "국가 개발여건 보조 신호", "보조 신호로 표시" if wdi_role_visible else "직접근거 오해 가능", "PASS" if wdi_role_visible else "BLOCK"],
        ["출처 URL·수집일", "미등록 상태 명시", "일부 원자료 메타데이터 미등록", "REVIEW"],
...
def render_builder(data):
    ...
    if clicked:
        ...
        local_proposal = build_rag_markdown_proposal(country, sector, user_type, scale, keywords, row, rag_docs, weights, design_assumptions)
        ...
        evidence_pack = build_rag_evidence_pack(country, sector, keywords, rag_docs, design_assumptions) + generation_metadata
        brief = build_policy_brief(country, sector, row, rag_docs, design_assumptions) + generation_metadata
        ...
        quality_status, quality_report = builder_output_quality_report(country, sector, rag_docs, proposal, brief, evidence_pack, proposal_pdf)
        ...
        if quality_status == "BLOCK":
            ...
            st.warning("산출물 자동 품질검사: REVIEW · 통합된 반복 레코드와 잠정 설계 가정, 미등록 메타데이터를 확인하세요.")
        else:

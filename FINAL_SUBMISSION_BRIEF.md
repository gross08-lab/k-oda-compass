...
| 심사 관점 | 제출물의 답 |
|---|---|
| 공공데이터 활용성 | KOICA 2019-2024 사업 레코드 12,436건, WDI 500행 중 유효 최신값 481건, CPS PDF 27개국·1,100개 chunk, 정책·실행환경 보조지표와 분야 포트폴리오를 결합 |
...
| 실사용성 | CSO, 지자체, 기업, 정책담당자가 국가·분야·근거·위험요인·KPI·Brief를 바로 확보 |
...
- KOICA 사업 근거: 12,436건
- WDI 최신 지표: 기대 500행 중 유효 최신값 481건, 10개 지표
- CPS PDF 근거: 27개국, 901개 검색 가능 페이지, 1,100개 유효 RAG 청크
- CPS 페이지 감사: 27개 PDF·921페이지 중 직접 추출 652페이지, OCR 검색 249페이지, 미검색 20페이지
...
4. `AI·모델 검증`에서 점수 재현성, RAG 근거 단위, CPS PDF 커버리지, OCR 검색 페이지, 민감도 분석을 확인합니다.
5. `Brief MD`, `Evidence Pack MD`, `Proposal PDF`, `Brief PDF`를 내려받아 실제 공모·사업기획 제출물 형태로 확인합니다.
...
- GitHub `gross08-lab/k-oda-compass`와 Streamlit Cloud 배포 URL은 앱·README·QR에 반영되어 있습니다.
- 현재 검증 변경은 사용자 승인 후 `main`에 일반 push하고 Streamlit 자동 재배포를 확인합니다.
- OpenAI API 키는 선택사항이며, 없거나 호출이 실패하면 Local RAG로 동작합니다.
